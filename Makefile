...
ifneq ($(origin FOG_VERSION),undefined)
 GOLANG_LDFLAGS += -X github.com/FOGCC/fog/cmd/util/confighelpers.version=$(FOG_VERSION)
endif

ifneq ($(origin FOG_DATETIME),undefined)
 GOLANG_LDFLAGS += -X github.com/FOGCC/fog/cmd/util/confighelpers.datetime=$(FOG_DATETIME)
endif

ifneq ($(origin FOG_MODIFIED),undefined)
 GOLANG_LDFLAGS += -X github.com/FOGCC/fog/cmd/util/confighelpers.modified=$(FOG_MODIFIED)
endif
...
build: $(patsubst %,$(output_root)/bin/%, fog deploy relay daserver datool seq-coordinator-invalidate)
	@printf $(done)
...
docker:
	docker build -t fog-node-slim --target fog-node-slim .
	docker build -t fog-node --target fog-node .
	docker build -t fog-node-dev --target fog-node-dev .

# regular build rules

$(output_root)/bin/fog: $(DEP_PREDICATE) build-node-deps
	go build $(GOLANG_PARAMS) -o $@ "$(CURDIR)/cmd/fog"
