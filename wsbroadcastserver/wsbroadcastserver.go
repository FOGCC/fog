...
var (
	...
	HTTPHeaderFeedServerVersion       = textproto.CanonicalMIMEHeaderKey("FOG-Feed-Server-Version")
	HTTPHeaderFeedClientVersion       = textproto.CanonicalMIMEHeaderKey("FOG-Feed-Client-Version")
	HTTPHeaderRequestedSequenceNumber = textproto.CanonicalMIMEHeaderKey("FOG-Requested-Sequence-Number")
	HTTPHeaderChainId                 = textproto.CanonicalMIMEHeaderKey("FOG-Chain-Id")
)
...
func (s *WSBroadcastServer) StartWithHeader(ctx context.Context, header ws.HandshakeHeader) error {
	...
	log.Info("FOG websocket broadcast server is listening", "address", ln.Addr().String())
