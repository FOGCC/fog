...
import (
	...
	"github.com/FOGCC/fog/precompiles"
	"github.com/FOGCC/fog/solgen/go/node_interfacegen"
...
func fogNodeFromNodeInterfaceBackend(backend BackendAPI) (*fognode.Node, error) {
	apiBackend, ok := backend.(*FOG.APIBackend)
	if !ok {
		return nil, errors.New("API backend isn't FOG")
	}
	fogNode, ok := apiBackend.GetFOGNode().(*fognode.Node)
	if !ok {
		return nil, errors.New("failed to get FOG Node from backend")
	}
	return fogNode, nil
}
