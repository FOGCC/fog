...
import (
	...
	"github.com/FOGCC/fog/util/containers"
	"github.com/FOGCC/fog/util/fogmath"
	"github.com/FOGCC/fog/util/headerreader"
	...
	"github.com/ethereum/go-ethereum/FOG"
...
var (
	sequencerBacklogGauge            = metrics.NewRegisteredGauge("fog/sequencer/backlog", nil)
	nonceCacheHitCounter             = metrics.NewRegisteredCounter("fog/sequencer/noncecache/hit", nil)
	nonceCacheMissCounter            = metrics.NewRegisteredCounter("fog/sequencer/noncecache/miss", nil)
	nonceCacheRejectedCounter        = metrics.NewRegisteredCounter("fog/sequencer/noncecache/rejected", nil)
	nonceCacheClearedCounter         = metrics.NewRegisteredCounter("fog/sequencer/noncecache/cleared", nil)
	nonceFailureCacheSizeGauge       = metrics.NewRegisteredGauge("fog/sequencer/noncefailurecache/size", nil)
	nonceFailureCacheOverflowCounter = metrics.NewRegisteredGauge("fog/sequencer/noncefailurecache/overflow", nil)
	blockCreationTimer               = metrics.NewRegisteredTimer("fog/sequencer/block/creation", nil)
	successfulBlocksCounter          = metrics.NewRegisteredCounter("fog/sequencer/block/successful", nil)
)
...
func (s *Sequencer) PublishTransaction(parentCtx context.Context, tx *types.Transaction) error {
	...
	if tx.Type() >= types.FOGDepositTxType {
		// Should be unreachable due to UnmarshalBinary not accepting FOG internal txs
...
func (s *Sequencer) postTxFilter(header *types.Header, _ *fogosState.fogosState, tx *types.Transaction, sender common.Address, dataGas uint64, result *core.ExecutionResult) error {
	if result.Err != nil && result.UsedGas > dataGas && result.UsedGas-dataGas <= s.config().MaxRevertGasReject {
		return FOG.NewRevertReason(result)
	}
	newNonce := tx.Nonce() + 1
