...
var (
	sourcesConnectedGauge    = metrics.NewRegisteredGauge("fog/feed/sources/connected", nil)
	sourcesDisconnectedGauge = metrics.NewRegisteredGauge("fog/feed/sources/disconnected", nil)
)
...
func (bc *BroadcastClient) connect(ctx context.Context, nextSeqNum fogutil.MessageIndex) (io.Reader, error) {
	...
	log.Info("connecting to FOG inbox message broadcaster", "url", bc.websocketUrl)
